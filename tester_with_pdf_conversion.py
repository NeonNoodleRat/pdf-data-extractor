...
if __name__ == "__main__":
    # Process the first PDF found in documents/ folder
    ...
    if not os.path.exists(docs_folder):
        ...
        if not pdf_files:
            ...
            pdf_path = os.path.join(docs_folder, first_pdf)
            print(f"Found PDF: {pdf_path}")
            # Convert first page to image
            image_paths = convert_pdf_to_images(pdf_path, output_folder="images")
            if not image_paths:
                print("Failed to convert PDF to image.")
            else:
                image_path = image_paths[0]
                print(f"Converted to image: {image_path}")
                # Test extraction on the generated image
